  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-3 border-b border-surface-100">
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-blue-700 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-full">
        ...
        {NAV.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            ...
            className={({ isActive }) => clsx(
              ...
              isActive ? 'bg-brand-700 text-white shadow-sm' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100'
            )}
          >
            {({ isActive }) => (
              <>
      ...
      <div className="px-3 py-4 border-t border-surface-100 space-y-1">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface-50">
          <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
            <span className="text-xs font-bold text-blue-700">{user?.name?.charAt(0) || 'S'}</span>
          </div>
